...
module.exports = async client => {
    ...
    (await pGlob(`${process.cwd()}/events/*/*.js`)).map(async eventFile => {
        ...
        if(!event.name) {
            return logger.warn(`Evenement non-déclenché: Aucun nom! Fichier => ${eventFile}`);
        }
        if(!eventList.includes(event.name)) {
            return logger.typo(`Evenement non-déclenché: Aucun nom! Fichier => ${eventFile}`);
        }
        if(event.once) {
        ...
        logger.event(`Evenement chargé: ${event.name}.js`);
    });
